...
for query in queries:
    searched_tweets = [status for status in tweepy.Cursor(api.search, q=query,since="2015-11-24",until="2015-11-25",lang=lang).items(MAX_TWEETS)]
...
with io.open(time.strftime("%d_%m_%Y") + '.json','w',encoding='utf-8')  as output:
    output.write(unicode(json.dumps(mlist, ensure_ascii=False, encoding='utf8')))
